...
# Convert matrices to displayable strings
def matrix_to_string(matrix):
    """Convert a matrix to a formatted string."""
    return "\n".join(["[" + " ".join(f"{num:2}" for num in row) + "]" for row in matrix])

invertible_2x2_strings = [matrix_to_string(matrix) for matrix in invertible_2x2_matrices]
invertible_3x3_strings = [matrix_to_string(matrix) for matrix in invertible_3x3_matrices]
...
# Select a Pre-validated Plaintext Matrix
if matrix_size == 2:
    matrix_string = st.selectbox("Select a Pre-validated Plaintext Matrix (Optional)", invertible_2x2_strings)
    selected_matrix = invertible_2x2_matrices[invertible_2x2_strings.index(matrix_string)]
else:
    matrix_string = st.selectbox("Select a Pre-validated Plaintext Matrix (Optional)", invertible_3x3_strings)
    selected_matrix = invertible_3x3_matrices[invertible_3x3_strings.index(matrix_string)]
